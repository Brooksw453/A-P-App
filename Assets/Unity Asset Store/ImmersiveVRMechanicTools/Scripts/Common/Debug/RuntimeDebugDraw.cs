...
namespace ReliableSolutions.Unity.Common.Debug
{
	public static class DebugDraw
	{
		...
		{
			Ray(transform.position, transform.up, Color.green, length);
			Ray(transform.position, transform.right, Color.red, length);
			Ray(transform.position, transform.forward, Color.blue, length);
		}
		...
		public static void Cube(Vector3 pos, Color col, Vector3 scale)
		{
			...
			UnityEngine.Debug.DrawLine(points[0], points[1], col);
			UnityEngine.Debug.DrawLine(points[1], points[2], col);
			UnityEngine.Debug.DrawLine(points[2], points[3], col);
			UnityEngine.Debug.DrawLine(points[3], points[0], col);
		}
		...
		public static void Point(Vector3 pos, Color col, float scale)
		{
			...
			UnityEngine.Debug.DrawLine(points[0], points[1], col);
			UnityEngine.Debug.DrawLine(points[2], points[3], col);
			UnityEngine.Debug.DrawLine(points[4], points[5], col);

			UnityEngine.Debug.DrawLine(points[0], points[2], col);
			UnityEngine.Debug.DrawLine(points[0], points[3], col);
			UnityEngine.Debug.DrawLine(points[0], points[4], col);
			UnityEngine.Debug.DrawLine(points[0], points[5], col);

			UnityEngine.Debug.DrawLine(points[1], points[2], col);
			UnityEngine.Debug.DrawLine(points[1], points[3], col);
			UnityEngine.Debug.DrawLine(points[1], points[4], col);
			UnityEngine.Debug.DrawLine(points[1], points[5], col);

			UnityEngine.Debug.DrawLine(points[4], points[2], col);
			UnityEngine.Debug.DrawLine(points[4], points[3], col);
			UnityEngine.Debug.DrawLine(points[5], points[2], col);
			UnityEngine.Debug.DrawLine(points[5], points[3], col);
		...
		{
			Text(pos, text, DrawDefaultColor, DrawTextDefaultSize, 0f, false);
			return;
		}
		...
		{
			Text(pos, text, color, DrawTextDefaultSize, 0f, false);
			return;
		}
		...
		{
			Text(pos, text, color, size, 0f, false);
			return;
		}
		...
		{
			Text(pos, text, color, size, duration, false);
			return;
		}
...
namespace RuntimeDebugDraw.Internal
{
	internal class RuntimeDebugDraw : MonoBehaviour
	{
		...
		private void TickAndDrawLines()
		{
			...
			Graphics.DrawMesh(_AlwaysBatch.mesh, Vector3.zero, Quaternion.identity, _AlwaysBatch.mat, layer: DebugDraw.DrawLineLayer, camera: null, submeshIndex: 0, properties: null, castShadows: false, receiveShadows: false);
			Graphics.DrawMesh(_ZTestBatch.mesh, Vector3.zero, Quaternion.identity, _ZTestBatch.mat, layer: DebugDraw.DrawLineLayer, camera: null, submeshIndex: 0, properties: null, castShadows: false, receiveShadows: false);
		...
		{
			var camera = DebugDraw.GetDebugDrawCamera();
			if (camera == null)
				return;
		...
		{
			if (!(Camera.current == DebugDraw.GetDebugDrawCamera()
